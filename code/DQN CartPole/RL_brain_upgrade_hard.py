...
class DeepQNetwork:
    def __init__(
        ...
        self.zero_table = self.q_table.copy()
        self.match_table = pd.DataFrame(columns=[0])
        self.zero_match_table = self.match_table.copy()
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            # first layer. collections is used later when assign to target net ***************************
            with tf.variable_scope('l1.5'):
                w15 = tf.get_variable('w1.5', [n_l1, n_l15], initializer=w_initializer, collections=c_names)
                b15 = tf.get_variable('b1.5', [1, n_l15], initializer=b_initializer, collections=c_names)
                l15 = tf.nn.relu(tf.matmul(l1, w15) + b15)

            with tf.variable_scope('l1.6'):
                w16 = tf.get_variable('w1.6', [n_l15, n_l16], initializer=w_initializer, collections=c_names)
                b16 = tf.get_variable('b1.6', [1, n_l16], initializer=b_initializer, collections=c_names)
                l16 = tf.nn.relu(tf.matmul(l15, w16) + b16)

            # second layer. collections is used later when assign to target net
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l16, self.n_actions], initializer=w_initializer, collections=c_names)
                b2 = tf.get_variable('b2', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                self.q_eval = tf.matmul(l16, w2) + b2
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l16, self.n_actions], initializer=w_initializer, collections=c_names)
                b2 = tf.get_variable('b2', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                self.q_next = tf.matmul(l16, w2) + b2
    ...
    def remember(self, s, a, r, s_):
        ...
        #self.check_state_exist(self.to_str(s_))
        #self.eligibility_trace.loc[self.to_str(s),:] *= 0
    ...
    def restore(self):
        for index, row in self.q_table.iterrows():
            if row[0] == 0:
                ...
                action = 0
            #if not np.array(self.match_table.loc[index, 0]):
            #    continue
